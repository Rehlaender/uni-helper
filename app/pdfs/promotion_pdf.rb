class PromotionPdf < Prawn::Document
  ...
  def header
    text "Reporte de todos los promotionos", size: 15, style: :bold
  end
  ...
  def table_content
    ...
    table promotion_rows do
      row(0).font_style = :bold
      self.header = true
      self.row_colors = ['DDDDDD', 'FFFFFF']
      self.column_widths = [35,30, 310, 75, 35]
    end
  end

  def promotion_rows
    [['Id', 'P-Id', 'PostName', 'Date', 'Paid']] +
      @promotions.map do |promotion|
      [promotion.id, promotion.post_id, promotion.post.description, promotion.created_at.to_s, promotion.paid.to_s]
    end
  end
end
